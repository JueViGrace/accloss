package com.clo.accloss.factura.domain.mappers

import com.clo.accloss.factura.data.remote.model.FacturaItem
import com.clo.accloss.factura.domain.model.Factura
import com.clo.accloss.Factura as FacturaEntity

fun FacturaEntity.toDomain(): Factura = Factura(
    aceptadev = aceptadev,
    agencia = agencia,
    bsflete = bsflete,
    bsiva = bsiva,
    bsmtofte = bsmtofte,
    bsmtoiva = bsmtoiva,
    bsretencioniva = bsretencioniva,
    cbsret = cbsret,
    cbsretflete = cbsretflete,
    cbsretiva = cbsretiva,
    cbsrparme = cbsrparme,
    cdret = cdret,
    cdretflete = cdretflete,
    cdretiva = cdretiva,
    cdrparme = cdrparme,
    codcliente = codcliente,
    codcoord = codcoord,
    contribesp = contribesp,
    dFlete = dFlete,
    diascred = diascred,
    documento = documento,
    dretencion = dretencion,
    dretencioniva = dretencioniva,
    dtotalfinal = dtotalfinal,
    dtotdescuen = dtotdescuen,
    dtotdev = dtotdev,
    dtotimpuest = dtotimpuest,
    dtotneto = dtotneto,
    dtotpagos = dtotpagos,
    dvndmtototal = dvndmtototal,
    emision = emision,
    estatusdoc = estatusdoc,
    fchvencedcto = fchvencedcto,
    fechamodifi = fechamodifi,
    ktiNegesp = ktiNegesp,
    mtodcto = mtodcto,
    nombrecli = nombrecli,
    recepcion = recepcion,
    retmunMto = retmunMto,
    rutaParme = rutaParme,
    tasadoc = tasadoc,
    tienedcto = tienedcto,
    tipodoc = tipodoc,
    tipodocv = tipodocv,
    tipoprecio = tipoprecio,
    vence = vence,
    vendedor = vendedor,
    empresa = empresa
)

fun Factura.toDatabase(): FacturaEntity = FacturaEntity(
    aceptadev = aceptadev,
    agencia = agencia,
    bsflete = bsflete,
    bsiva = bsiva,
    bsmtofte = bsmtofte,
    bsmtoiva = bsmtoiva,
    bsretencioniva = bsretencioniva,
    cbsret = cbsret,
    cbsretflete = cbsretflete,
    cbsretiva = cbsretiva,
    cbsrparme = cbsrparme,
    cdret = cdret,
    cdretflete = cdretflete,
    cdretiva = cdretiva,
    cdrparme = cdrparme,
    codcliente = codcliente,
    codcoord = codcoord,
    contribesp = contribesp,
    dFlete = dFlete,
    diascred = diascred,
    documento = documento,
    dretencion = dretencion,
    dretencioniva = dretencioniva,
    dtotalfinal = dtotalfinal,
    dtotdescuen = dtotdescuen,
    dtotdev = dtotdev,
    dtotimpuest = dtotimpuest,
    dtotneto = dtotneto,
    dtotpagos = dtotpagos,
    dvndmtototal = dvndmtototal,
    emision = emision,
    estatusdoc = estatusdoc,
    fchvencedcto = fchvencedcto,
    fechamodifi = fechamodifi,
    ktiNegesp = ktiNegesp,
    mtodcto = mtodcto,
    nombrecli = nombrecli,
    recepcion = recepcion,
    retmunMto = retmunMto,
    rutaParme = rutaParme,
    tasadoc = tasadoc,
    tienedcto = tienedcto,
    tipodoc = tipodoc,
    tipodocv = tipodocv,
    tipoprecio = tipoprecio,
    vence = vence,
    vendedor = vendedor,
    empresa = empresa
)

fun FacturaItem.toDomain(): Factura = Factura(
    aceptadev = aceptadev ?: "",
    agencia = agencia ?: "",
    bsflete = bsflete ?: 0.0,
    bsiva = bsiva ?: 0.0,
    bsmtofte = bsmtofte ?: 0.0,
    bsmtoiva = bsmtoiva ?: 0.0,
    bsretencioniva = bsretencioniva ?: 0.0,
    cbsret = cbsret ?: 0.0,
    cbsretflete = cbsretflete ?: 0.0,
    cbsretiva = cbsretiva ?: 0.0,
    cbsrparme = cbsrparme ?: 0.0,
    cdret = cdret ?: 0.0,
    cdretflete = cdretflete ?: 0.0,
    cdretiva = cdretiva ?: 0.0,
    cdrparme = cdrparme ?: 0.0,
    codcliente = codcliente ?: "",
    codcoord = codcoord ?: "",
    contribesp = contribesp ?: 0.0,
    dFlete = dFlete ?: 0.0,
    diascred = diascred ?: 0.0,
    documento = documento ?: "",
    dretencion = dretencion ?: 0.0,
    dretencioniva = dretencioniva ?: 0.0,
    dtotalfinal = dtotalfinal ?: 0.0,
    dtotdescuen = dtotdescuen ?: 0.0,
    dtotdev = dtotdev ?: 0.0,
    dtotimpuest = dtotimpuest ?: 0.0,
    dtotneto = dtotneto ?: 0.0,
    dtotpagos = dtotpagos ?: 0.0,
    dvndmtototal = dvndmtototal ?: 0.0,
    emision = emision ?: "",
    estatusdoc = estatusdoc ?: "",
    fchvencedcto = fchvencedcto ?: "",
    fechamodifi = fechamodifi ?: "",
    ktiNegesp = ktiNegesp ?: "",
    mtodcto = mtodcto ?: 0.0,
    nombrecli = nombrecli ?: "",
    recepcion = recepcion ?: "",
    retmunMto = retmunMto ?: 0.0,
    rutaParme = rutaParme ?: "",
    tasadoc = tasadoc ?: 0.0,
    tienedcto = tienedcto ?: "",
    tipodoc = tipodoc ?: "",
    tipodocv = tipodocv ?: "",
    tipoprecio = tipoprecio ?: 0.0,
    vence = vence ?: "",
    vendedor = vendedor ?: ""
)
